...
using System.Linq;
    ...
    public class ControllerGenerator: MyApplicationModelController
    {
        ...
        public Dictionary<string, string> GetWebApi()
        {
            ...
            foreach (var p in CreateModels().controllers)
            {
        ...
        public Dictionary<string, string> GetJsApi()
        {
            ...
            foreach (Type controllerType in controllers)
            {
                ...
                try
                {
                    api[controllerType.GetTypeName()] = GenerateAngularJsService(controllerType);
                }catch (Exception ex)
                {
                    ex.ToString().ToJsonOnScreen();
                }
            } 
            return api;
        }
        ...
        /// <returns></returns>
        public string GenerateAngularJsService(string typeName)
        {
            Type type = ReflectionService.TypeForName(typeName);
            return this.GenerateAngularJsService(type);
        }
        ...
        public string AngularJsService(MyControllerModel controllerModel)
        {
            ...
            if (HasBaseType(ReflectionService.TypeForName(controllerModel.Name), typeof(BaseController)))
            {
    ...
    private bool HasBaseType(Type type1, Type type2)
    {
